...
class Toplist {
    ...
    updateList() {
        ...
        exec('powershell -NoProfile -OutputFormat Text -InputFormat Text -Command "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; Get-Process | Select-Object Id,ProcessName,CPU,WorkingSet,StartTime | ConvertTo-Json"', { encoding: 'utf8' }, 
            (error, stdout, stderr) => {
                if (error) {
    ...
    processList(){
        ...
        const updateProcessList = () => {
            ...
            exec('powershell -NoProfile -OutputFormat Text -InputFormat Text -Command "[Console]::OutputEncoding = [System.Text.Encoding]::UTF8; Get-Process | Select-Object Id,ProcessName,CPU,WorkingSet,StartTime,Responding,SessionId,Handles,Threads,Path | ConvertTo-Json"', { encoding: 'utf8' }, 
                (error, stdout, stderr) => {
                    if (error) {
                        console.error('获取进程信息失败:', error);
