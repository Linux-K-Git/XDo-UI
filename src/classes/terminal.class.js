    ...
    constructor(opts) {
        if (opts.role === "client") {
            ...
            this.term = new this.xTerm({
                ...
                fontFamily: (window.theme.terminal.fontFamily || "Fira Mono") + ', "Microsoft YaHei", "SimHei", monospace',
                ...
                return new Promise((resolve, reject) => {
                    ...
                    switch(require("os").type()) {
                        ...
                        case "Darwin":
                            ...
                            break;
                        case "Windows_NT":
                            // Windows平台暂不支持TTY工作目录跟踪
                            // Windows platform does not support TTY working directory tracking yet
                            reject("Windows平台暂不支持工作目录跟踪 (Windows platform does not support working directory tracking)");
                            break;
                        default:
                            reject(`不支持的操作系统: ${require("os").type()} (Unsupported OS: ${require("os").type()})`);
                    }
                });
            };
                ...
                return new Promise((resolve, reject) => {
                    ...
                    switch(require("os").type()) {
                        ...
                        case "Darwin":
                            ...
                            break;
                        case "Windows_NT":
                            // Windows平台暂不支持TTY子进程检索
                            // Windows platform does not support TTY subprocess retrieval yet
                            reject("Windows平台暂不支持子进程检索 (Windows platform does not support subprocess retrieval)");
                            break;
                        default:
                            reject(`不支持的操作系统: ${require("os").type()} (Unsupported OS: ${require("os").type()})`);
                    }
            ...
            this._tick = setInterval(() => {
                if (this._nextTickUpdateTtyCWD && this._disableCWDtracking === false) {
                    ...
                    this._getTtyCWD(this.tty).then(cwd => {
                        ...
                        if (!this._closed) {
                            console.log("跟踪TTY工作目录时出错 (Error while tracking TTY working directory): ", e);
                ...
                if (this.renderer && this._nextTickUpdateProcess) {
                    ...
                    this._getTtyProcess(this.tty).then(process => {
                        ...
                        if (!this._closed) {
                            console.log("检索TTY子进程时出错 (Error while retrieving TTY subprocess): ", e);
                            try {
            ...
            this.tty = this.Pty.spawn(opts.shell || "bash", (opts.params.length > 0 ? opts.params : (process.platform === "win32" ? [] : ["--login"])), {
                ...
                encoding: "utf8"  // 确保使用UTF-8编码 (Ensure UTF-8 encoding)
            });
