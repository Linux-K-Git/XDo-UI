...
function initSystemInformationProxy() {
    ...
    // 设置Windows平台的编码环境 (Set encoding environment for Windows platform)
...
function displayLine() {
    ...
    switch(true) {
        case i === 2:
            bootScreen.innerHTML += `XDo-UI Kernel version ${remote.app.getVersion()} boot at ${Date().toString()}; root:xnu-1699.22.73~1/RELEASE_X86_64`;
        case i === 4:
...
async function displayTitleScreen() {
    ...
    bootScreen.innerHTML = "<h1>XDo-UI</h1>";
...
async function initUI() {
    ...
    window.term[0].term.writeln("\x1b[1m"+`Welcome to XDo-UI v${remote.app.getVersion()} - Electron v${process.versions.electron}`+"\x1b[0m");
    ...
    window.windowSelector = new WindowSelector();
    
    // Initialize particle background
    window.particleBackground = new ParticleBackground();
    window.particleBackground.setIntensity(0.3); // 设置适中的粒子强度
}
...
window.openSettings = async () => {
    ...
    new Modal({
        ...
        buttons: [
            ...
            {label: "Restart XDo", action: "remote.app.relaunch();remote.app.quit();"}
        ]
    }, () => {
