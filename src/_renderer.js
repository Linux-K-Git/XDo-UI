...
function displayLine() {
    ...
    switch(true) {
        case i === 2:
            bootScreen.innerHTML += `XDo-UI Kernel version ${electron.remote.app.getVersion()} boot at ${Date().toString()}; root:xnu-1699.22.73~1/RELEASE_X86_64`;
        case i === 4:
...
async function initUI() {
    ...
    window.term[0].term.writeln("\033[1m"+`Welcome to XDo-UI v${electron.remote.app.getVersion()} - Electron v${process.versions.electron}`+"\033[0m");
...
window.openSettings = async () => {
    ...
    new Modal({
        ...
        buttons: [
            ...
            {label: "Restart XDo", action: "electron.remote.app.relaunch();electron.remote.app.quit();"}
        ]
    }, () => {
