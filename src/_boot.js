...
process.on("uncaughtException", e => {
    signale.fatal(e);
    dialog.showErrorBox("XDo-UI crashed", e.message || "Cannot retrieve error message.");
    if (tty) {
        tty.close();
    }
    if (extraTtys) {
...
signale.start(`Starting XDo-UI v${app.getVersion()}`);
signale.info(`With Node ${process.versions.node} and Electron ${process.versions.electron}`);
signale.info(`Renderer is Chrome ${process.versions.chrome}`);

const gotLock = app.requestSingleInstanceLock();
if (!gotLock) {
    signale.fatal("Error: Another instance of XDo is already running. Cannot proceed.");
    app.exit(1);
}
...
if (!fs.existsSync(settingsFile)) {
    ...
    let defaultIface = "以太网";
    try {
        // 修正为同步调用方式
        const interfaces = si.networkInterfacesSync();
        ...
        if (ethInterface) {
            defaultIface = ethInterface.iface;
        }
    } catch (e) {
        signale.warn("网络接口自动检测失败，使用备用值");
    }
...
function createWindow(settings) {
    signale.info("正在创建窗口 (Creating window)...");
    ...
    signale.complete("前端窗口创建完成 (Frontend window created)!");
    win.show();
    ...
    signale.watch("等待前端连接 (Waiting for frontend connection)...");
}

app.on('ready', async () => {
    signale.pending(`正在加载设置文件 (Loading settings file)...`);
    let settings = require(settingsFile);
    signale.pending(`正在解析Shell路径 (Resolving shell path)...`);
    settings.shell = await which(settings.shell).catch(e => { throw(e) });
    signale.info(`Shell路径: ${settings.shell} (Shell found at ${settings.shell})`);
    signale.success(`设置加载完成 (Settings loaded)!`);
    ...
    let shellArgs = settings.shellArgs || [];
    if (typeof shellArgs === 'string') {
        shellArgs = shellArgs.split(' ').filter(arg => arg.length > 0);
    }
    
    if (process.platform === 'win32' && settings.shell.toLowerCase().includes('powershell')) {
        if (shellArgs.length === 0) {
             // Use more robust arguments for PowerShell UTF-8
             shellArgs = ['-NoLogo', '-NoExit', '-Command', '[Console]::OutputEncoding=[System.Text.Encoding]::UTF8; chcp 65001 > $null'];
        }
    }

    signale.pending(`正在创建终端进程，端口: ${settings.port || '3000'} (Creating new terminal process on port ${settings.port || '3000'})`);
    tty = new Terminal({
        role: "server",
    ...
    signale.success(`终端后端初始化完成 (Terminal back-end initialized)!`);
    tty.onclosed = (code, signal) => {
        tty.ondisconnected = () => {};
        signale.complete("终端已退出 (Terminal exited)", code, signal);
        app.quit();
    };
    tty.onopened = () => {
        signale.success("已连接到前端 (Connected to frontend)!");
        signale.timeEnd("Startup");
    };
    tty.onresized = (cols, rows) => {
        signale.info("TTY尺寸已调整为 (Resized TTY to) ", cols, rows);
    };
    tty.ondisconnected = () => {
        signale.error("与前端连接丢失 (Lost connection to frontend)");
        signale.watch("等待前端连接 (Waiting for frontend connection)...");
    };

    // Support for multithreaded systeminformation calls
    signale.pending("正在启动多线程调用控制器 (Starting multithreaded calls controller)...");
