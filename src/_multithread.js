...
if (cluster.isMaster) {
    ...
    signale.success("多线程控制器就绪 (Multithreaded controller ready)");
    ...
    ipc.on("systeminformation-call", (e, type, id, ...args) => {
        if (!si[type]) {
            signale.warn("非法的系统信息请求 (Illegal request for systeminformation)");
            return;
        }
    ...
    // 设置进程编码为UTF-8 (Set process encoding to UTF-8)
    ...
    signale.info("多线程工作进程已启动 (Multithread worker started at) "+process.pid);
